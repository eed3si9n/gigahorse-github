...
implicit lazy val RepoFormat: JsonFormat[gigahorse.github.response.Repo] = new JsonFormat[gigahorse.github.response.Repo] {
  override def read[J](jsOpt: Option[J], unbuilder: Unbuilder[J]): gigahorse.github.response.Repo = {
    jsOpt match {
      ...
      unbuilder.endObject()
      new gigahorse.github.response.Repo(url, name, id, owner, full_name, description, `private`, fork, html_url, clone_url, git_url, ssh_url, homepage, language, forks_count, watchers_count, size, default_branch, open_issues_count, pushed_at, created_at, updated_at)
      case None =>
