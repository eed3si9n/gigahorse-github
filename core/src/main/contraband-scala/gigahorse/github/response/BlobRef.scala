...
final class BlobRef private (
  ...
  private def this(url: String, sha: String, name: String, path: String, repository: gigahorse.github.response.Repo) = this(url, sha, name, path, repository, None, None)
  ...
  override def hashCode: Int = {
    37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (17 + "BlobRef".##) + url.##) + sha.##) + name.##) + path.##) + repository.##) + git_url.##) + html_url.##)
  }
  override def toString: String = {
    "BlobRef(" + url + ", " + sha + ", " + name + ", " + path + ", " + repository + ", " + git_url + ", " + html_url + ")"
  }
  protected[this] def copy(url: String = url, sha: String = sha, name: String = name, path: String = path, repository: gigahorse.github.response.Repo = repository, git_url: Option[String] = git_url, html_url: Option[String] = html_url): BlobRef = {
...
object BlobRef {
  ...
  def apply(url: String, sha: String, name: String, path: String, repository: gigahorse.github.response.Repo, git_url: String, html_url: String): BlobRef = new BlobRef(url, sha, name, path, repository, Option(git_url), Option(html_url))
}
